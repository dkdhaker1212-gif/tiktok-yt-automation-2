...
- **Per upload:** download watermark-free → verify audio → blur + light de-dup edit
  ...
  upload as public Short → record in `data/channel_2.db` (committed back) → Discord ping

## Layout

```
channels.yaml            THE config (schema: Automation.md §5)
run.py                   entry: python run.py --slot N --channel channel_2 [--dry-run]
...
|---|---|---|
| `CHANNEL_2_CLIENT_SECRET` | base64 of `credentials/channel_2_client_secret.json` | upload |
| `CHANNEL_2_TOKEN` | base64 of `tokens/channel_2_token.json` | upload |
...
python run.py --slot 1 --channel channel_2 --dry-run
```
